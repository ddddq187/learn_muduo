...
public:
  ...
  explicit Buffer(size_t initalSize = kInitialSize)
      : buffer_(kCheapPrepend + initalSize), readerIndex_(kCheapPrepend),
  ...
  std::string retrieveAsString(size_t len) {
    ...
    retrieve(len); // 上面一句把缓冲区中可读的数据已经读取出来
                   // 这里肯定要对缓冲区进行复位操作
    return result;
  }
...
private:
  ...
  void makeSpace(size_t len) {
     ...
     * xxx标示reader中已读的部分 | kCheapPrepend | reader ｜          len    |
     **/
    if (writableBytes() + prependableBytes() <
        len + kCheapPrepend) // 也就是说 len > xxx前面剩余的空间 + writer的部分
    {
      buffer_.resize(writerIndex_ + len);
    } else // 这里说明 len <= xxx + writer 把reader搬到从xxx开始
           // 使得xxx后面是一段连续空间
    {
