...
void TcpConnection::sendInLoop(const void *data, size_t len) {
  ...
  if (state_ ==
      kDisconnected) // 之前调用过该connection的shutdown 不能再进行发送了
  {
  ...
  if (!channel_->isWriting() && outputBuffer_.readableBytes() == 0) {
    ...
    if (nwrote > = 0) {
      ...
      nwrote = 0;
      if (errno !=
          EWOULDBLOCK) // EWOULDBLOCK表示非阻塞情况下没有数据后的正常返回
                       // 等同于EAGAIN
      {
  ...
  // 读是相对服务器而言的 当对端客户端有数据到达 服务器端检测到EPOLLIN
    ...
    if (channel_->isWriting()) {
      ...
      if (n > 0) {
        outputBuffer_.retrieve(
            n); // 从缓冲区读取reable区域的数据移动readindex下标
    ...
    if (connected()) {
      if (loop_->isInLoopThread()) { // 判断当前线程是否是loop循环的线程
        ...
        loop_->runInLoop(std::bind(&TcpConnection::sendFileInLoop,
                                   shared_from_this(), fileDescriptor, offset,
                                   count));
      }
    } else {
  ...
  // 在事件循环中执行sendfile
  void TcpConnection::sendFileInLoop(int fileDescriptor, off_t offset,
                                     size_t count) {
    ...
    if (!channel_->isWriting() && outputBuffer_.readableBytes() == 0) {
      ...
      if (bytesSent >= 0) {
        ...
        if (errno !=
            EWOULDBLOCK) { // 如果是非阻塞没有数据返回错误这个是正常显现等同于EAGAIN，否则就异常情况
